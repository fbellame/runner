...
- **The Xcode project is generated by XcodeGen** from `project.yml` (folder-based sources, `createIntermediateGroups: true`). `Runner.xcodeproj/` is gitignored and MUST NOT be committed. After creating or deleting any `.swift` file, run `xcodegen generate` to regenerate the project so the new file is included. Never hand-edit `project.pbxproj`. Only commit source files (and `project.yml` if it changes — it won't for plain file additions).
- Tests run in the **main session's shell only** (the iOS simulator/CoreSimulatorService is not reachable from sandboxed subagents): `xcodebuild test -scheme Runner -destination 'platform=iOS Simulator,name=iPhone 17'` (run `xcrun simctl list devices available` first if that device is absent, and substitute an available simulator). Scope with `-only-testing:RunnerTests/<Suite>` while iterating.

---

### Task 1: Confirm the XcodeGen file-registration workflow

The project is generated by XcodeGen from `project.yml`, whose targets use folder-based sources (`sources: [Runner]`, `sources: [RunnerTests]`) with `createIntermediateGroups: true`. New `.swift` files placed anywhere under those folders are picked up automatically on the next `xcodegen generate` — no manual `project.pbxproj` editing. This task removes the mistaken pbxproj helper and verifies the real workflow.

**Files:**
- Delete: `scripts/xcadd.rb` (mis-registered files at a doubled path; the project is generated, so it is unnecessary and harmful).

- [ ] **Step 1: Confirm xcodegen is available**

Run: `xcodegen --version`
Expected: prints a version (e.g. `Version: 2.45.4`). If missing: `brew install xcodegen`.

- [ ] **Step 2: Remove the mistaken helper**

Run: `git rm -f scripts/xcadd.rb`

- [ ] **Step 3: Verify regeneration works and includes a scratch file**

```bash
echo "// scratch" > Runner/Core/_ScratchProbe.swift
xcodegen generate
```
Confirm the generated project references the scratch file (basename path, not doubled):
`grep -c "_ScratchProbe.swift" Runner.xcodeproj/project.pbxproj` → expect `2` (one PBXBuildFile + one PBXFileReference), and the PBXFileReference line shows `path = _ScratchProbe.swift; sourceTree = "<group>"` (basename only). Then clean up:
```bash
rm Runner/Core/_ScratchProbe.swift
xcodegen generate
```
...
```bash
git add -A
git commit -m "build: use xcodegen generate for file registration (project is XcodeGen-generated)"
```
...
Run: `xcodebuild test -scheme Runner -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:RunnerTests/CalorieEngineTests`
...
```bash
git add Runner/Core/CalorieEngine RunnerTests/CalorieEngineTests.swift
...
Run: `xcodebuild test -scheme Runner -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:RunnerTests/DataStoreTests`
...
`xcodebuild test -scheme Runner -destination 'platform=iOS Simulator,name=iPhone 17'`
...
Run: `xcodebuild test -scheme Runner -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:RunnerTests/ProfileStoreTests`
...
```bash
git add Runner/Core/Health Runner/Core/Profile RunnerTests/FakeHealthStore.swift RunnerTests/ProfileStoreTests.swift
...
Run: `xcodebuild test -scheme Runner -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:RunnerTests/SyncCoordinatorTests`
...
xcodegen generate
```
Run: `xcodebuild build -scheme Runner -destination 'platform=iOS Simulator,name=iPhone 17'`
...
```bash
git add Runner/Features/Profile Runner/Features/Settings/SettingsView.swift
...
```bash
git add Runner/Features/History
...
Run: `xcodebuild test -scheme Runner -destination 'platform=iOS Simulator,name=iPhone 17'`
